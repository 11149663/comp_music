...
### **|New|**: Predicting my music listening behaviour over the years
...
### 2: Topping the artist charts from UK to Taiwan {data-commentary-width = 500}
...
listens <- scrobbles %>%
  ...
  ggtitle("Number of 'scrobbles' per year including Top 5") +
  ...
  geom_vline(xintercept = 4.65, linetype = 2, size = 0.3, alpha = 0.3, color = "red") +
    annotate(geom = "text", x = 4.65, y = 13500, label = "<b>Jan 2015</b>:\nNew Phone + Spotify", size = 3) +

  geom_vline(xintercept = 6.2, linetype = 2, size = 0.3, alpha = 0.3) +
    annotate(geom = "text", x = 6.2, y = 15000, label = "<b>Sep 2016</b>:\nStart UvA", size = 3) +

  geom_vline(xintercept = 8.1, linetype = 2, size = 0.3, alpha = 0.3) +
    annotate(geom = "text", x = 8.1, y = 11000, label = "<b>Aug 2018</b>:\nMoved to Hong Kong", size = 3) +
  
  geom_vline(xintercept = 9.8, linetype = 2, size = 0.3, alpha = 0.3) +
    annotate(geom = "text", x = 9.8, y = 9000, label = "<b>Mar 2020</b>:\nCovid-19 Pandemic", size = 3) +
  
  annotate(geom = "text", x = 1, y = 9073, label = "9373", size = 2.5) +
  annotate(geom = "text", x = 2, y = 7483, label = "7783", size = 2.5) +
  annotate(geom = "text", x = 3, y = 6301, label = "6601", size = 2.5) +
  annotate(geom = "text", x = 4, y = 7280, label = "7580", size = 2.5) +
  annotate(geom = "text", x = 5, y = 10278, label = "10578", size = 2.5) +
  annotate(geom = "text", x = 6, y = 17473, label = "17773", size = 2.5) +
  annotate(geom = "text", x = 7, y = 8999, label = "9299", size = 2.5) +
  annotate(geom = "text", x = 8, y = 7025, label = "7325", size = 2.5) +
  annotate(geom = "text", x = 9, y = 6085, label = "6385", size = 2.5) +
  annotate(geom = "text", x = 10, y = 4192, label = "4492", size = 2.5)
...
Looking at all my recorded scrobbles as a whole, you can observe that it increased
significantly in [2015](https://www.last.fm/user/wutsmainaem/library/artists?from=2015-01-01&rangetype=year), after which it peaked in [2016](https://www.last.fm/user/wutsmainaem/library/artists?from=2016-01-01&rangetype=year) at 17773. As I mentioned in the previous slide, this increase corresponds to me getting a new smartphone in 2015, which made it easier for me to use Spotify with a last.fm plugin (from 249 in December, 2014 to 1635 in January, 2015).
...
On a surface level, other interesting developments can be observed when looking at the top 5 most listened artists over the years. In 2011 and 2012, my top 5s were dominated by artists from the UK (9 out of 10). The top 5s of 2013/2014 diversified with artists coming from other places than the UK (4/10). From 2015 onwards, the top 5s can be typified in several ways: "noisy" music (Preoccupations/Aburadako/Death Grips) and math rock (tricot/No Party). My exposure to modern music from East Asia has had a noticeable effect on my top 5s starting from late 2018. This kind of music was so dominant that it overtook the entire top 5 in 2019 and most of 2018 and 2020.
...
2014/2: "CE middelbare school" (correspond to lower valence as seen in 2)
...
### 7.1: Average tempo similar over the years
...
A minor outlier is 2015-2016, which comes around 134 bpm. This coincidentally correlates to 2016 having
the highest energy (see 3.1).
...
A typical ("Unconscious Melody") and an atypical ("Setsuyakuka") song were 
selected to compare its tempograms. The tempo of Unconscious Melody is mostly constant
between 115-120 bpm. 

The tempo of the song "Setsuyakuka" is, however, less constant. Although the tempo is relatively
pronounced around 90 bpm, it is still highly varied. Interestingly, the tempo between 150-200 seconds
seems to be higher than the rest of the song. Listening to the song, it could be possible that 
the tempogram had a less hard time calculating the tempo, as that same section sounds more quiet than
the rest.
