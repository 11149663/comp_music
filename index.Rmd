...
#    pop %>% mutate(playlist = "Soul Pop") %>% slice_head(n = 20),
#    party %>% mutate(playlist = "Soul Party") %>% slice_head(n = 20),
#    workout %>% mutate(playlist = "Soul Workout") %>% slice_head(n = 20)
```
...
```{r, include = FALSE}
indie_knn %>% get_conf_mat() # get confusion matrix
```
...
```{r}
indie_knn %>% get_pr() # shows in percentages with 0.33 in this case is random chance
```
...
KNN matrix:
- Periods 2011-2012, 2013-2014 are most distinctive compared to others (to lesser extent: 2019-2020).
- 2017-2018 has a minor overlap with 2015-2016. Could mean that my listening habits were similar within those years.
- 

Random forest tree:
- Timbre is important (c01, c03, and c11).
- Instrumentalness is also an important factor. Could be due to math rock (usually without vocals in East Asia) and ambient music.
